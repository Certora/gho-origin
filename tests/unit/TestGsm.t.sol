...
contract TestGsm is TestGhoBase {
  ...
  function testConstructor() public {
    ...
    assertEq(gsm.UNDERLYING_ASSET(), address(USDX_TOKEN), 'Unexpected underlying asset address');
    assertEq(
      gsm.PRICE_STRATEGY(),
      address(GHO_GSM_FIXED_PRICE_STRATEGY),
  ...
  function testRevertConstructorInvalidPriceStrategy() public {
    ...
    new Gsm(address(GHO_TOKEN), address(USDX_TOKEN), address(newPriceStrategy));
  }

  function testRevertConstructorZeroAddressParams() public {
    vm.expectRevert('ZERO_ADDRESS_NOT_VALID');
    new Gsm(address(0), address(USDX_TOKEN), address(GHO_GSM_FIXED_PRICE_STRATEGY));
  ...
  function testSellAssetZeroFee() public {
    ...
    (uint256 assetAmount, uint256 ghoBought) = GHO_GSM.sellAsset(DEFAULT_GSM_USDX_AMOUNT, ALICE);
    vm.stopPrank();
    ...
    assertEq(GHO_GSM.getExposureCap(), DEFAULT_GSM_USDX_EXPOSURE, 'Unexpected exposure capacity');
  }
  ...
  function testSellAssetSendToOther() public {
    ...
    (uint256 assetAmount, uint256 ghoBought) = GHO_GSM.sellAsset(DEFAULT_GSM_USDX_AMOUNT, BOB);
    vm.stopPrank();
  ...
  function testSellAssetWithSig() public {
    ...
    assertEq(USDX_TOKEN.balanceOf(gsmSignerAddr), 0, 'Unexpected final USDX balance');
  ...
  function testRevertSellAssetWithSigInvalidSignature() public {
    ...
    GHO_GSM.sellAssetWithSig(ALICE, DEFAULT_GSM_USDX_AMOUNT, ALICE, deadline, signature);
  }
  ...
  function testRevertSellAssetNoBucketCap() public {
    ...
    vm.startPrank(ALICE);
    USDX_TOKEN.approve(address(gsm), defaultCapInUsdx);
  ...
  function testRevertSellAssetTooMuchUnderlyingExposure() public {
    ...
    vm.prank(FAUCET);
    USDX_TOKEN.mint(ALICE, DEFAULT_GSM_USDX_EXPOSURE);

    vm.startPrank(ALICE);
    USDX_TOKEN.approve(address(gsm), DEFAULT_GSM_USDX_EXPOSURE);
    vm.expectRevert('EXOGENOUS_ASSET_EXPOSURE_TOO_HIGH');
    gsm.sellAsset(DEFAULT_GSM_USDX_EXPOSURE, ALICE);
    vm.stopPrank();
  }
  ...
  function testGetGhoAmountForSellAsset() public {
    (uint256 exactAssetAmount, uint256 ghoBought, uint256 grossAmount, uint256 fee) = GHO_GSM
      .getGhoAmountForSellAsset(DEFAULT_GSM_USDX_AMOUNT);

    _sellAsset(GHO_GSM, USDX_TOKEN, ALICE, DEFAULT_GSM_USDX_AMOUNT);

    assertEq(
      DEFAULT_GSM_USDX_AMOUNT - USDX_TOKEN.balanceOf(ALICE),
      exactAssetAmount,
    ...
    assertEq(assetAmount, DEFAULT_GSM_USDX_AMOUNT, 'Unexpected estimation of sold assets');
  ...
  function testBuyAssetZeroFee() public {
    ...
    (uint256 assetAmount, uint256 ghoSold) = GHO_GSM.buyAsset(DEFAULT_GSM_USDX_AMOUNT, BOB);
    vm.stopPrank();

    assertEq(ghoSold, DEFAULT_GSM_GHO_AMOUNT, 'Unexpected GHO amount sold');
    assertEq(assetAmount, DEFAULT_GSM_USDX_AMOUNT, 'Unexpected asset amount bought');
  ...
  function testBuyAssetSendToOther() public {
    ...
    assertEq(ghoSold, DEFAULT_GSM_GHO_AMOUNT + buyFee, 'Unexpected GHO amount sold');
    assertEq(assetAmount, DEFAULT_GSM_USDX_AMOUNT, 'Unexpected asset amount bought');
    assertEq(USDX_TOKEN.balanceOf(BOB), 0, 'Unexpected final USDX balance');
    assertEq(
      USDX_TOKEN.balanceOf(CHARLES),
      DEFAULT_GSM_USDX_AMOUNT,
  ...
  function testBuyAssetWithSig() public {
    ...
    assertEq(GHO_GSM.nonces(gsmSignerAddr), 1, 'Unexpected final gsmSignerAddr nonce');
    assertEq(
      USDX_TOKEN.balanceOf(gsmSignerAddr),
      DEFAULT_GSM_USDX_AMOUNT,
      'Unexpected final USDX balance'
    );
    assertEq(GHO_TOKEN.balanceOf(ALICE), ghoOut, 'Unexpected final GHO balance');
  ...
  function testBuyThenSellAtMaximumBucketCapacity() public {
    ...
    USDX_TOKEN.mint(ALICE, DEFAULT_GSM_USDX_EXPOSURE);
    vm.startPrank(ALICE);
    USDX_TOKEN.approve(address(GHO_GSM), DEFAULT_GSM_USDX_EXPOSURE);
    vm.expectEmit(true, true, true, true, address(GHO_GSM));
    emit SellAsset(ALICE, ALICE, DEFAULT_GSM_USDX_EXPOSURE, DEFAULT_CAPACITY, 0);
    GHO_GSM.sellAsset(DEFAULT_GSM_USDX_EXPOSURE, ALICE);
    ...
    assertEq(USDX_TOKEN.balanceOf(ALICE), 1e6, 'Unexpected Alice USDX balance after buy');

    // Sell 1 of the underlying
    USDX_TOKEN.approve(address(GHO_GSM), 1e6);
    vm.expectEmit(true, true, true, true, address(GHO_GSM));
    ...
    assertEq(USDX_TOKEN.balanceOf(ALICE), 0, 'Unexpected Alice USDX balance after second sell');
    assertEq(GHO_GSM.getExposureCap(), DEFAULT_GSM_USDX_EXPOSURE, 'Unexpected exposure capacity');
  }
  ...
  function testRevertBuyAssetWithSigInvalidSignature() public {
    ...
    vm.prank(BOB);
    vm.expectRevert('SIGNATURE_INVALID');
    GHO_GSM.buyAssetWithSig(BOB, DEFAULT_GSM_USDX_AMOUNT, gsmSignerAddr, deadline, signature);
  }
  ...
  function testRevertBuyAssetNoGHO() public {
    ...
    GHO_GSM.buyAsset(DEFAULT_GSM_USDX_AMOUNT, BOB);
    vm.stopPrank();
  }
  ...
  function testGetGhoAmountForBuyAsset() public {
    (uint256 exactAssetAmount, uint256 ghoSold, uint256 grossAmount, uint256 fee) = GHO_GSM
      .getGhoAmountForBuyAsset(DEFAULT_GSM_USDX_AMOUNT);
    ...
    assertEq(DEFAULT_GSM_USDX_AMOUNT, exactAssetAmount, 'Unexpected asset amount bought');
    ...
    assertEq(assetAmount, DEFAULT_GSM_USDX_AMOUNT, 'Unexpected estimation of bought assets');
    assertEq(grossAmount, grossAmount2, 'Unexpected GHO gross amount');
    assertEq(fee, fee2, 'Unexpected GHO fee amount');
  }
  ...
  function testRescueUnderlyingTokens() public {
    ...
    vm.prank(FAUCET);
    USDX_TOKEN.mint(address(GHO_GSM), DEFAULT_GSM_USDX_AMOUNT);

    assertEq(USDX_TOKEN.balanceOf(ALICE), 0, 'Unexpected USDX balance before');
    vm.expectEmit(true, true, true, true, address(GHO_GSM));
    emit TokensRescued(address(USDX_TOKEN), ALICE, DEFAULT_GSM_USDX_AMOUNT);
    GHO_GSM.rescueTokens(address(USDX_TOKEN), ALICE, DEFAULT_GSM_USDX_AMOUNT);
    assertEq(USDX_TOKEN.balanceOf(ALICE), DEFAULT_GSM_USDX_AMOUNT, 'Unexpected USDX balance after');
  }
  ...
  function testRescueUnderlyingTokensWithAccruedFees() public {
    ...
    uint256 currentGSMBalance = DEFAULT_GSM_USDX_AMOUNT;
    assertEq(
      USDX_TOKEN.balanceOf(address(GHO_GSM)),
      currentGSMBalance,
      'Unexpected GSM USDX balance before'
    );
    ...
    assertEq(
      USDX_TOKEN.balanceOf(address(GHO_GSM)),
      currentGSMBalance + DEFAULT_GSM_USDX_AMOUNT,
      'Unexpected GSM USDX balance before, post-mint'
    );
    assertEq(USDX_TOKEN.balanceOf(ALICE), 0, 'Unexpected target USDX balance before');
    ...
    assertEq(
      USDX_TOKEN.balanceOf(address(GHO_GSM)),
      currentGSMBalance,
      'Unexpected GSM USDX balance after'
    );
    assertEq(
      USDX_TOKEN.balanceOf(ALICE),
      DEFAULT_GSM_USDX_AMOUNT,
      'Unexpected target USDX balance after'
    );
  }
  ...
  function testRevertRescueUnderlyingTokens() public {
    ...
    GHO_GSM.rescueTokens(address(USDX_TOKEN), ALICE, 1);
  }
  ...
  function testSeize() public {
    ...
    assertEq(USDX_TOKEN.balanceOf(TREASURY), 0, 'Unexpected USDX before token balance');
    ...
    assertEq(
      USDX_TOKEN.balanceOf(TREASURY),
      DEFAULT_GSM_USDX_AMOUNT,
      'Unexpected USDX after token balance'
    );
    assertEq(GHO_GSM.getAvailableLiquidity(), 0, 'Unexpected available liquidity');
    assertEq(
      GHO_GSM.getAvailableUnderlyingExposure(),
  ...
  function testUpdateExposureCapBelowCurrentExposure() public {
    assertEq(GHO_GSM.getExposureCap(), DEFAULT_GSM_USDX_EXPOSURE, 'Unexpected exposure cap');
